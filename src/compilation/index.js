...
export default (zstyles, screenWidth) => {
    ...
    stylesArray.forEach(style => options.components[style] && options.components[style].split(' ').forEach(object => stylesArray.push(object)));
    let { screens } = options;
    let screenStyles = Object.keys(screens).reduce((carry, screen) => {
        let screenStyles = stylesArray.filter(style => style.startsWith(screen + ':'));
        if (screenStyles.length > 0) {
            let screenObject = {};
            screenObject.width = screens[screen];
            screenObject.screen = screen;
            screenObject.styles = screenStyles.map(style => style.replace(screen + ':', ''));
    ...
    return finalStylesArray.map(style => {
