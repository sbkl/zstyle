...
export default class ZImage extends Component {
	...
	componentDidMount() {
		...
		setTimeout(() => {
			if (animated && CachedWith) {
				...
				this.props.zref(
					this._component
						? this._component.refs == null
							? this._component
							: this._component.refs.cachedImage
							? this._component.refs.cachedImage._viewRef
							: this._component._root
						: null
				)
			} else if (animated) {
				this.props.zref(this._component._component)
	...
	render() {
		let { style, animated, CachedWith, ...rest } = this.props
		if (animated && CachedWith) {
			return (
				<Animated.View
			...
			return (
				<CachedWith
					onLayout={this.compileStyles}
					ref={component => (this._component = component)}
					style={[{ width: "100%", height: "100%" }, this.state.zstyles, style]}
					{...rest}
				/>
			)
		} else if (animated) {
